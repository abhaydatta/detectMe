...
class FaceBoundsOverlay @JvmOverloads constructor(ctx: Context, attrs: AttributeSet? = null) :
    ...
    init {
        anchorPaint.color = ContextCompat.getColor(context, R.color.yellow)

        idPaint.color = ContextCompat.getColor(context, R.color.yellow)
        idPaint.textSize = 40f

        boundsPaint.style = Paint.Style.STROKE
        boundsPaint.color = ContextCompat.getColor(context, R.color.yellow)
